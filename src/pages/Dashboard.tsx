...
const Dashboard = () => {
  ...
  const debounceTitleTimerRef = useRef<NodeJS.Timeout | null>(null);
  ...
  // Atualizar ref quando selectedNote muda
  useEffect(() => {
    selectedNoteRef.current = selectedNote;
  }, [selectedNote]);

  // Sincronizar conteúdo e título local quando a nota selecionada muda
  useEffect(() => {
    if (selectedNote) {
  ...
  }, [selectedNote?.id]); // Apenas quando a nota muda, não quando o conteúdo/título muda

  // Limpar timers ao desmontar ou mudar de nota
  useEffect(() => {
    return () => {
      if (debounceTimerRef.current) {
        clearTimeout(debounceTimerRef.current);
      }
      if (debounceTitleTimerRef.current) {
        clearTimeout(debounceTitleTimerRef.current);
      }
    };
  }, [selectedNote?.id]);
  ...
  const updateNote = async (updates: Partial<Note>, skipStateUpdate = false) => {
  ...
  // Handler para mudanças no conteúdo com debounce
  const handleContentChange = (value: string) => {
    // Atualizar estado local imediatamente para UI responsiva
    setLocalContent(value);
    
    // Atualizar estado da nota selecionada localmente (sem salvar no banco ainda)
    setSelectedNote(prevNote => {
      if (!prevNote) return prevNote;
      const updatedNote = { ...prevNote, content: value };
      setNotes(prevNotes => prevNotes.map(n => n.id === prevNote.id ? updatedNote : n));
      return updatedNote;
    });
    ...
    // Salvar no banco após 1 segundo de inatividade
  ...
  // Handler para mudanças no título com debounce
  const handleTitleChange = (value: string) => {
    ...
    setSelectedNote(prevNote => {
      if (!prevNote) return prevNote;
      const updatedNote = { ...prevNote, title: value };
      setNotes(prevNotes => prevNotes.map(n => n.id === prevNote.id ? updatedNote : n));
      return updatedNote;
    });
    ...
    // Salvar no banco após 1 segundo de inatividade
    debounceTitleTimerRef.current = setTimeout(async () => {
      const currentNote = selectedNoteRef.current;
      if (currentNote) {
        const { error } = await supabase
          .from("notes")
  ...
  const generateTags = async () => {
    ...
    try {
      ...
      await updateNote({ tags: generatedTags });
      toast({
        title: "Tags generated",
        description: `Added ${generatedTags.length} tags to your note`,
      });
    } catch (error) {
      toast({
